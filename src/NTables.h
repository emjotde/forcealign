...
public:
	...
	VALTYPE getValue(int w, unsigned int n) const {
		...
		if (w>=ntab.getLen1()) {
		    if(n == 0)
			return max(float(1.0/pow(2,10)), VALTYPE(PROB_SMOOTH));
		    else
			return max(float(1.0/pow(2,n)), VALTYPE(PROB_SMOOTH));
		}
		else {
			VALTYPE temp = ntab(w, n);
			return max(temp, VALTYPE(PROB_SMOOTH));
		}
	}
protected:
...
public:
	inline void addValue(int w , int n,const VALTYPE& t){
		lock.lock();
		if (w < ntab.getLen1() && n < ntab.getLen2()) 
			ntab(w,n) += t;
		lock.unlock();};
public:
	template<class COUNT> void normalize(nmodel<COUNT>&write,
		...
		if (_evlist&&(NTablesFactorGraphemes||NTablesFactorGeneral)) {
			...
			std::cerr << "EVLIST: " << std::endl;
			std::cerr << evlist.size() << std::endl;
